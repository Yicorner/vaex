...
class Args(Tap):
    ...
    record_reconstruction_metadata: bool = True  # Save run_metadata.json under local_out_dir_path
    ...
    ep: int = 250               # epochs
    lbs: int = 8                # local batch size (exclusive to --bs) if this is specified, --bs will be ignored
...
def init_dist_and_get_args():
    ...
    if not args.lpips_path:
        args.lpips_path = f'./ckpt_vaex/vgg.pth'
    if not args.dino_path:
        args.dino_path = f'./ckpt_vaex/vit_small_patch16_224_dino.pth'
    if not args.val_img_pattern:
        args.val_img_pattern = f'./ckpt_vaex/val_imgs/v*'
